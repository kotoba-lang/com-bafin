(ns bafin.main
  "Kotodama WASM entrypoint for the Bafin clean-room actor (L4) — Clojure port.
  ...
  facts (`bafin.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "bafin_fil"
    :fields [:filerId :scheme :period :status]   :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "bafin_cit"
    :fields [:nationalId :jurisdiction]          :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "bafin_ser"
    :fields [:name :department]                  :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "bafin_app"
    :fields [:citizenId :serviceId :status]      :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "bafin_dat"
    :fields [:name :publisher :updatedAt]        :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "bafin_ide"
...
(defn emit-facts
  "EAVT facts for one record: {\"bafin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bafin-compat" :tier tier :entities entities} 200])
